...
def main():
    ...
    # Initialize Manager
    manager = LonelyManager(agent_id="manager-alpha", todo_path="TASKS.md")
    manager.start()
    ...
    # Set to RELAXING so they are picked up by the task assigner
    from vaultwares_agentciation.enums import AgentStatus
    text_p.update_status(AgentStatus.RELAXING)
    image_p.update_status(AgentStatus.RELAXING)
    video_p.update_status(AgentStatus.RELAXING)
