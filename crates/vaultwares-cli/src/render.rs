...
impl ColorTheme {
    pub fn from_vaultwares_theme(slug: &str) -> Option<Self> {
        let palette = palette_by_slug(slug).ok()?;
        Some(Self {
            heading: parse_hex_color(palette.accent)?,
            emphasis: parse_hex_color(palette.focus_ring)?,
            strong: parse_hex_color(palette.accent)?,
            inline_code: parse_hex_color(palette.text_secondary)?,
            link: parse_hex_color(palette.accent_hover)?,
            quote: parse_hex_color(palette.border_subtle)?,
            table_border: parse_hex_color(palette.border_subtle)?,
            code_block_border: parse_hex_color(palette.border_subtle)?,
            spinner_active: parse_hex_color(palette.accent)?,
            spinner_done: Color::Green,
            spinner_failed: Color::Red,
        })
    }
