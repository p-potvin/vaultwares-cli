...
use supervisor::{
    LaneRegistration, LaneState, LaneSupervisor, SupervisorConfig, TerminalLauncher,
    TerminalTarget, WindowsTerminalLauncher, WorkerLaunchSpec,
};
use tools::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::SupervisorStatus {
            redis_url,
            output_format,
        } => run_supervisor_status(&redis_url, output_format)?,
        CliAction::SpawnVisibleWorker {
            redis_url,
            lane_id,
            worker_name,
            cwd,
            output_format,
        } => run_spawn_visible_worker(&redis_url, &lane_id, &worker_name, &cwd, output_format)?,
        CliAction::Worker {
            redis_url,
            lane_id,
            worker_name,
            heartbeat_secs,
        } => run_worker_loop(&redis_url, &lane_id, &worker_name, heartbeat_secs)?,
        CliAction::Prompt {
            prompt,
            model,
            output_format,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match rest[0].as_str() {
        ...
        "supervisor" => parse_supervisor_args(&rest[1..], output_format),
        "worker" => parse_worker_args(&rest[1..]),
...
fn parse_supervisor_args(
        ...
        while index < args.len() {
            match args[index].as_str() {
                "--redis-url" => {
                    ...
                    let value = args
                        .get(index)
                        .ok_or_else(|| "missing value for --redis-url".to_string())?;
                    redis_url = value.clone();
                }
                other => {
                    return Err(format!("unsupported supervisor status argument `{other}`"));
                }
            }
            index += 1;
        }
        return Ok(CliAction::SupervisorStatus {
            redis_url,
            output_format,
        });
    }

    if args[0] != "spawn-visible" {
    ...
    Ok(CliAction::SpawnVisibleWorker {
        redis_url,
        lane_id,
        worker_name,
        cwd,
...
fn parse_worker_args(args: &[String]) -> Result<CliAction, String> {
    let mut redis_url = "redis://127.0.0.1/".to_string();
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--lane-id" => {
                ...
                lane_id = Some(
                    args.get(index)
                        .ok_or_else(|| "missing value for --lane-id".to_string())?
                        .clone(),
                );
            }
            "--worker-name" => {
                ...
                worker_name = Some(
                    args.get(index)
                        .ok_or_else(|| "missing value for --worker-name".to_string())?
                        .clone(),
                );
            }
            "--heartbeat-secs" => {
                ...
                heartbeat_secs = args
                    .get(index)
                    .ok_or_else(|| "missing value for --heartbeat-secs".to_string())?
                    .parse::<u64>()
                    .map_err(|_| "heartbeat seconds must be an integer".to_string())?;
            }
    ...
    Ok(CliAction::Worker {
        redis_url,
        lane_id: lane_id.ok_or_else(|| "worker requires --lane-id".to_string())?,
        worker_name: worker_name.ok_or_else(|| "worker requires --worker-name".to_string())?,
        heartbeat_secs,
    })
}
...
fn run_supervisor_status(
    redis_url: &str,
    output_format: CliOutputFormat,
) -> Result<(), Box<dyn std::error::Error>> {
    let windows_terminal_available = Command::new("where")
        .arg("wt")
        .output()
    ...
    let payload = json!({
        ...
        "status_channel": "vaultwares:status",
        "events_stream": "vw:coord:v1:events",
    });
    match output_format {
        CliOutputFormat::Json => println!("{}", serde_json::to_string_pretty(&payload)?),
...
fn run_spawn_visible_worker(
    ...
    let supervisor = LaneSupervisor::new(config)?;
    ...
    let launcher = WindowsTerminalLauncher;
    let spawned = supervisor.spawn_visible_lane(
        &launcher,
        spec,
        "worker".to_string(),
        Vec::new(),
        ...
        CliOutputFormat::Json => {
            println!(
                ...
                serde_json::to_string_pretty(&json!({
                    ...
                    "pid": spawned.process.child.id(),
                    "cwd": cwd.display().to_string(),
                }))?
            );
        }
        CliOutputFormat::Text => {
            println!("Spawned visible worker");
            println!("  Lane ID     {lane_id}");
            println!("  Worker      {worker_name}");
            println!("  PID         {}", spawned.process.child.id());
...
fn run_worker_loop(
    ...
    config.redis_url = redis_url.to_string();
    let supervisor = LaneSupervisor::new(config)?;
    let _ = supervisor.register_lane(LaneRegistration {
        lane_id: lane_id.to_string(),
        worker_name: worker_name.to_string(),
        cwd: env::current_dir()?,
        ...
        terminal_target: TerminalTarget::WindowsTerminalTab,
        catalog_revision: Some("bootstrap".to_string()),
        worker_id: Some(format!("worker_{lane_id}")),
    })?;
    println!(
        "VaultWares worker online: lane={} worker={} redis={}",
        lane_id, worker_name, redis_url
    );
    loop {
        let _ = supervisor.record_heartbeat(lane_id, LaneState::Idle, None)?;
